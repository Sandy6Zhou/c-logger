...
static void logger_setAppName(char *name){
    if(strlen(name) >= sizeof(app_name)){
        fprintf(stderr, "ERROR: logger: Failed to set app name: `%s`\n", name);
        return ;
    }
    strcpy(app_name, name);
}

static char* logger_getAppName(void){
	return app_name;
}

static long vflog(FILE* fp, char* levelc, const char* timestamp, const char* app_name,
        ...
        unsigned long long currentTime, unsigned long long* flushedTime)
{
    ...
    if ((size = fprintf(fp, "[%s][%s][%s %s:%d] ", levelc, timestamp, app_name, file, line)) > 0) {
        totalsize += size;
    }
...
void logger_log(LogLevel level, const char* file, int line, const char* fmt, ...)
{
    ...
    if (hasFlag(s_logger, kConsoleLogger)) {
        va_start(carg, fmt);
        vflog(s_clog.output, levels, timestamp, logger_getAppName(),
    ...
    if (hasFlag(s_logger, kFileLogger)) {
        if (rotateLogFiles()) {
            va_start(farg, fmt);
            s_flog.currentFileSize += vflog(s_flog.output, levels, timestamp, logger_getAppName(),
